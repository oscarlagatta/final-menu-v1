...
// Update the props interface to include selectedMetricType
type LeaderPerformanceProps = {
  ...
  selectedMetricType?: { id?: number; name?: string | null } | null
}
...
export default function LeaderPerformance({
  ...
  // Update the title to show which month's data is being displayed
  return (
    <Card>
      <CardHeader className="pb-2">
        {/* Update the title to show the selected metric type */}
        <CardTitle className="text-xl font-bold">
          Leader Performance {selectedLeader ? `- ${selectedLeader.name}` : ""}
          {selectedMetricType ? ` - ${selectedMetricType.name}` : ""}
          {selectedMonth ? ` (${selectedMonth})` : " (All Months - showing data from " + defaultMonth + ")"}
        </CardTitle>
        <CardDescription>
      ...
      <CardContent>
        ...
        <div className="mb-6">
          ...
          {(() => {
            const totalSum = totals.green + totals.amber + totals.red
            return (
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div
                          className="h-8 bg-green-600 text-white flex items-center justify-center rounded-l-md cursor-help"
                          style={{
                            width: `${(totals.green / totalSum) * 100}%`,
                      ...
                      <TooltipContent side="top" className="max-w-md p-4">
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {Math.round((totals.green / totalSum) * 100)}% of total metrics
                          </p>
                        </div>
                      </TooltipContent>
                    </Tooltip>
                ...
                {totals.amber > 0 && (
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div
                          className={`h-8 bg-amber-500 text-white flex items-center justify-center ${
                            totals.green === 0 ? "rounded-l-md" : ""
                          } ${totals.red === 0 ? "rounded-r-md" : ""} cursor-help`}
                          style={{
                            width: `${(totals.amber / totalSum) * 100}%`,
                      ...
                      <TooltipContent side="top" className="max-w-md p-4">
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {Math.round((totals.amber / totalSum) * 100)}% of total metrics
                          </p>
                        </div>
                      </TooltipContent>
                    </Tooltip>
                ...
                {totals.red > 0 && (
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div
                          className={`h-8 bg-red-600 text-white flex items-center justify-center ${
                            totals.green === 0 && totals.amber === 0 ? "rounded-l-md" : ""
                          } rounded-r-md cursor-help`}
                          style={{
                            width: `${(totals.red / totalSum) * 100}%`,
                            minWidth: totals.red > 0 ? "40px" : "0",
                          }}
                        >
                          {totals.red}
                        </div>
                      ...
                      <TooltipContent side="top" className="max-w-md p-4">
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {Math.round((totals.red / totalSum) * 100)}% of total metrics
                          </p>
                        </div>
                      </TooltipContent>
                    </Tooltip>
          ...
          {leaderData.length > 0 ? (
            leaderData.map((leader) => (
              <div key={leader.id} className="mb-4">
                ...
                {/* Calculate total metrics for this leader */}
                {(() => {
                  const leaderTotal = leader.green + leader.amber + leader.red
                  return (
                    <div className="flex items-center w-full">
                      {leader.green > 0 && (
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div
                                className="h-8 bg-green-600 text-white flex items-center justify-center rounded-l-md cursor-help"
                                style={{
                                  width: `${(leader.green / leaderTotal) * 100}%`,
                            ...
                            <TooltipContent side="top" className="max-w-md p-4">
                              <div className="space-y-2">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {Math.round((leader.green / leaderTotal) * 100)}% of total metrics
                                </p>
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div
                                className={`h-8 bg-amber-500 text-white flex items-center justify-center ${
                                  leader.green === 0 ? "rounded-l-md" : ""
                                } ${leader.red === 0 ? "rounded-r-md" : ""} cursor-help`}
                                style={{
                                  width: `${(leader.amber / leaderTotal) * 100}%`,
                            ...
                            <TooltipContent side="top" className="max-w-md p-4">
                              <div className="space-y-2">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {Math.round((leader.amber / leaderTotal) * 100)}% of total metrics
                                </p>
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div
                                className={`h-8 bg-red-600 text-white flex items-center justify-center ${
                                  leader.green === 0 && leader.amber === 0 ? "rounded-l-md" : ""
                                } rounded-r-md cursor-help`}
                                style={{
                                  width: `${(leader.red / leaderTotal) * 100}%`,
                            ...
                            <TooltipContent side="top" className="max-w-md p-4">
                              <div className="space-y-2">
                                ...
                                <p className="text-xs text-muted-foreground">Metrics below limit performance</p>
                                <p className="text-xs text-muted-foreground">
                                  {Math.round((leader.red / leaderTotal) * 100)}% of total metrics
                                </p>
