    ...
    const { totalMetrics, greenMetrics, amberMetrics, redMetrics, performanceRate, momChange, metricsWithColors } =
        useMemo(() => {
            ...
            const metricsWithColors = sixMonthByMetricPerformance.map((metric) => {
                const firstMonth_Color = determineColor(metric.firstMonth_Result, metric.trigger, metric.limit)
                const secondMonth_Color = determineColor(metric.secondMonth_Result, metric.trigger, metric.limit)
                const thirdMonth_Color = determineColor(metric.thirdMonth_Result, metric.trigger, metric.limit)
                const fourthMonth_Color = determineColor(metric.fourthMonth_Result, metric.trigger, metric.limit)
                const fiveMonth_Color = determineColor(metric.fiveMonth_Result, metric.trigger, metric.limit)
                const sixthMonth_Color = determineColor(metric.sixthMonth_Result, metric.trigger, metric.limit)

                // Generate a metricPrefix from the metricId if not available
                const metricPrefix = `PM${String(metric.metricId).padStart(3, "0")}`
                ...
                return {
                    ...
                    fiveMonth_Color,
                    sixthMonth_Color,
                    metricColor: firstMonth_Color, // Current month color
                }
            })
