...
const MetricsGridFiltered = ({ selectedMonth, selectedLeader }: MetricsGridProps) => {
  ...
  // Pass parameters in the correct order: selectedMonth first, then selectedLeader
  const {
  ...
  // Modified to filter columns based on selectedMonth
  const monthColumns = useMemo(() => {
    ...
    ].filter(m => m.month); // Filter out any undefined months
    
    // If a specific month is selected, only show that month
    if (selectedMonth) {
      return allMonths.filter(m => {
        // Convert both to YYYY-MM format for comparison
        const columnMonth = m.month?.substring(0, 7);
        return columnMonth === selectedMonth;
      });
    }
...
export default MetricsGridFiltered;
